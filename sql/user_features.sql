...
user_features_base AS (
    SELECT
        spu.user_id,
        spu.total_sessions,
        ROUND(tpu.completed_trips::decimal * 100 / spu.total_sessions, 2) AS conversion_rate,
        spu.avg_page_clicks,
...
user_features AS (
	SELECT *,
		--indices
		LOG(1 + total_sessions) * LOG(1 + avg_page_clicks) * LOG(1 + avg_session_duration_minutes) AS activity_index,
		LOG(1 + COALESCE(avg_revenue_per_trip, 0)) * LOG(1 + COALESCE(total_trips, 0)) * LOG(1 + COALESCE(revenue_per_session, 0)) AS value_index,
		LOG(1 + COALESCE(conversion_rate,0)) * LOG(1 + total_sessions) * LOG(1 + COALESCE(avg_revenue_per_trip,0)) AS quality_index
	FROM user_features_base
	WHERE total_sessions >= 5
),
percentiles AS (
    SELECT
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY activity_index) AS act_p25,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY activity_index) AS act_p75,
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY value_index) AS val_p25,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY value_index) AS val_p75
    FROM user_features
)
SELECT
    uf.*,
    CASE
        WHEN uf.value_index >= p.val_p75 AND uf.activity_index >= p.act_p75 THEN 'high_value_high_activity'
        WHEN uf.value_index >= p.val_p75 AND uf.activity_index < p.act_p75 THEN 'high_value_low_activity'
        WHEN uf.value_index < p.val_p75 AND uf.activity_index >= p.act_p75 THEN 'low_value_high_activity'
        ELSE 'low_value_low_activity'
    END AS user_segment
FROM user_features uf
CROSS JOIN percentiles p
WHERE avg_lead_time IS NOT NULL 
ORDER BY 1 ASC;